            ...
            <ul className="hidden pb-4 text-sm font-medium text-center text-gray-500 rounded-lg shadow-sm sm:flex dark:divide-gray-700 dark:text-gray-400">
                {items.map(item => {
                    return <li key={item.value} className="w-full focus-within:z-10">
                        <a href={void (0)} className={`inline-block w-full p-2 border-gray-200 dark:border-gray-700 focus:outline-none dark:bg-gray-800 ${selectedValue === item.value ? 'bg-primary-light text-white dark:bg-primary-dark text-gray-900 dark:bg-gray-700 dark:text-white active' : ''} ${item.value === items[0].value ? 'rounded-s-lg' : ''} ${item.value === items[items.length - 1].value ? 'border-s-0 rounded-e-lg' : 'border-r'}`} onClick={() => { onSelectChange(item.value) }}>{item.text}</a>
                    </li>
                })}
            </ul>
        </>
